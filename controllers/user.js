...
module.exports.renderUserAccountPage = async (req, res) => {
    try {
        const orders = await orderModel.find({ userId: req.session.user.userId }, { orderNumber: 1, createdAt: 1, status: 1, grandTotal: 1, cancellationReason: 1 }).sort({ createdAt: -1 }).limit(4);
